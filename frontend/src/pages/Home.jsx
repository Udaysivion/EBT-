...
function FeaturedMenu({ categoriesData }) {
...
function FeaturedBranches({ branchesData }) {
  const { ref, inView } = useInView({ threshold: 0.1, triggerOnce: true })
  return (
...
export default function Home() {
  ...
  const [branchesState, setBranchesState] = useState(branches)

  useEffect(() => {
    dataService.getStats().then(data => { if (data) setStatsState(data) })
    dataService.getMenu().then(data => { if (data) setCategoriesState(data) })
    dataService.getBranches().then(data => { if (data) setBranchesState(data) })
  }, [])

  return (
    <div className="page-wrapper" style={{ paddingTop: 0 }}>
      <Hero />
      <StatsBar statsData={statsState} />
      <AboutSection />
      <FeaturedMenu categoriesData={categoriesState} />
      <FeaturedBranches branchesData={branchesState} />
      <FranchiseCTA />
    </div>
  )
}
